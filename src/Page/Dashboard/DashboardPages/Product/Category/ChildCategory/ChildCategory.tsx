...
import {
  ChildCategoryData,
  getAllChildData,
} from "../../../../../../http/childCategory";
import { useChildCategoryHandlers } from "../../../../../../hooks/useChildCategoryHandlers";
import BreadCrumbs from "../../../../../../comps/BreadCrumbs/BreadCrumbs";
import { HiArrowLongLeft, HiArrowLongRight } from "react-icons/hi2";

const ChildCategory = () => {
  const [data, setData] = useState<ChildCategoryData[]>([]);
  ...
  useEffect(() => {
    (async () => {
      try {
        const users = await getAllChildData(); // Fetch users
        if (!users) {
              ...
              <table className="table">
                <thead>
                  <tr>
                    ...
                    <th className="text-sm" style={{ width: "22%" }}>
                      Description
                    </th>
                    <th className="text-sm" style={{ width: "10%" }}>
                      Status
                ...
                <tbody>
                  {paginatedData.map((childCategory) => (
                    <tr key={childCategory.id}>
                      <td>{childCategory.id}</td>
                      <td>
                        <img
                          src={childCategory.child_category_icon}
                          alt="Icon"
                      ...
                      <td>
                        <img
                          src={childCategory.child_category_thumbnail}
                          alt="Thumbnail"
                      ...
                      <td>
                        <img
                          src={childCategory.child_category_banner}
                          alt="Banner"
                      ...
                      <td>{childCategory.child_category_description}</td>
                      <td>
                        <button
